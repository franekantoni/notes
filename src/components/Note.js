...
export default class Note extends React.Component{
	...
	openNewNote = () => {
		...
		const noteId = sha256((Date.now()*Math.random()*100).toString()).slice(0,14)
		newState.noteId = noteId
		newState.password = sha256((Date.now()*Math.random()*100).toString()).slice(0,10)
	...
	submitPassowrd = (password) => {
		...
		fetch('https://3lzatsbxak.execute-api.eu-central-1.amazonaws.com/beta/note?sk='+encodeURIComponent(hash)+'&pk='+encodeURIComponent(pk),
		{	
			method: 'GET',
		    headers: {
		      'Accept': 'application/json',
	...
	copy = () => {
		...
		navigator.clipboard.writeText(window.location.origin+'/'+this.state.noteId)
		setTimeout(()=>{
	...
	changeTime = (adding) => {
		...
		for (const {timeSegment, segmentLength} of [{timeSegment: days, segmentLength: oneDay}, {timeSegment: hours, segmentLength: oneHour}, {timeSegment: minutes, segmentLength: oneMinute}]){
			if (timeSegment > 0){
		    	if (adding){
		    		newUntilTimestamp = this.state.untilTimestamp+segmentLength;
		    	}
		    	else {
		    		newUntilTimestamp = Math.max(this.state.untilTimestamp-segmentLength, Date.now()+segmentLength)
		    	}
		    	break
	    	}
		}
	...
	startTimeChangeTimeout = () => {
		...
		this.timeChangeTimeout = setTimeout(()=>{
			fetch('https://3lzatsbxak.execute-api.eu-central-1.amazonaws.com/beta/time',
			{	
				method: 'POST',
			    headers: {
			      'Accept': 'application/json',
	...
	render(){
		return(
			<div style={{backgroundColor: '#282c34', minHeight: '100vh', color: 'white', padding: '1vw', display: 'flex', justifyContent: 'center', alignItems: 'center'}}>
			  {this.state.loaded &&
			    <div>
			      {this.state.savedText &&
				      <div style={{marginTop: '1vw', flexDirection: 'row', display: 'flex', justifyContent: 'space-between'}}>
				        <NoteIdentity
